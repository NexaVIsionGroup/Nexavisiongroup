...
export default function SigningClient({ agreement: initialAgreement }: {
  agreement: Agreement
}) {
  ...
  const submitSignature = async () => {
    if (!canvasRef.current || !activeSigner) return
    setSubmitting(true)
    setError('')
    try {
      ...
      const res = await fetch(`/api/sign/${agreement.id}`, {
        ...
        body: JSON.stringify({ signerNumber: activeSigner, signature: signatureData })
      })
      if (!res.ok) {
        const data = await res.json()
      ...
      setSuccessMsg(`Signature recorded for ${agreement[`signer_${activeSigner}_name`]}`)
  ...
  const signerInfo = (n: number) => ({
    name: agreement[`signer_${n}_name`] as string,
    role: agreement[`signer_${n}_role`] as string,
    signed: !!agreement[`signer_${n}_signature`],
    signedAt: agreement[`signer_${n}_signed_at`] as string | null,
    signature: agreement[`signer_${n}_signature`] as string | null,
  })

  const allSigned = [1,2,3,4].every(n => !!agreement[`signer_${n}_signature`])
        ...
        <div className="bg-white rounded-lg shadow-xl border border-gray-200 overflow-hidden">
          <div className="px-6 sm:px-10 py-8">
            ...
            <div className="mt-10 pt-6 border-t-2 border-[#C9A84C]">
              ...
              {successMsg && (
                <div className="bg-green-50 border border-green-300 rounded-md px-4 py-3 mb-6 text-center">
                  <p className="text-sm text-green-700 font-medium">✓ {successMsg}</p>
                </div>
              )}

              {allSigned && (
                <div className="bg-green-50 border-2 border-green-400 rounded-lg px-6 py-5 mb-6 text-center">
                  <p className="text-lg font-bold text-green-800">✓ All Parties Have Signed</p>
                  <p className="text-sm text-green-600 mt-1">This agreement is fully executed as of the dates indicated below.</p>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                {[1,2,3,4].map(n => {
                  const s = signerInfo(n)
                  return (
                    <div key={n} className={`border-2 rounded-lg p-4 transition-all ${
                      s.signed 
                        ? 'border-green-400 bg-green-50' 
                        : activeSigner === n 
                          ? 'border-[#1B2A4A] bg-[#FAFBFD] ring-2 ring-[#C9A84C]' 
                      ...
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {s.signed ? (
                          <span className="text-xs bg-green-200 text-green-800 px-3 py-1 rounded-full font-bold">✓ SIGNED</span>
                        ) : (
                      ...
                      {s.signed && s.signature && (
                        <div className="mt-2 border-t border-green-200 pt-2">
                          <img src={s.signature} alt={`${s.name} signature`} className="h-14 max-w-full" />
                          <p className="text-xs text-gray-400 mt-1">
                            Signed: {new Date(s.signedAt!).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: '2-digit' })}
                          </p>
                        </div>
                      )}

                      {/* Sign button for unsigned */}
                      {!s.signed && activeSigner !== n && (
                        <button
                          onClick={() => { setActiveSigner(n); setHasSigned(false); setError(''); setSuccessMsg('') }}
                          className="mt-2 w-full py-2 text-sm bg-[#1B2A4A] text-white rounded-md hover:bg-[#2a3f6a] transition font-medium"
                        >
                          Sign as {s.name}
                        </button>
                      )}
              ...
              {/* SIGNATURE PAD - appears when a signer is selected */}
              {activeSigner && (
                <div className="border-2 border-[#1B2A4A] rounded-lg p-6 bg-[#FAFBFD] mt-4">
                  <h4 className="text-lg font-bold text-[#1B2A4A] mb-1">
                    Signing as: {agreement[`signer_${activeSigner}_name`]}
                  </h4>
                  <p className="text-sm text-gray-500 mb-4">{agreement[`signer_${activeSigner}_role`]}</p>
                  
                  <p className="text-xs text-gray-400 mb-2">Draw your signature below:</p>
                  <div className="border-2 border-gray-300 rounded-md bg-white mb-3 relative">
                    <canvas
                      ref={canvasRef}
                      width={700}
                      height={180}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => { setActiveSigner(null); setHasSigned(false) }}
                      className="px-4 py-2 text-sm border border-gray-300 rounded-md hover:bg-gray-100 transition">
                      Cancel
                    </button>
...
function P({ children }: { children: React.ReactNode }) {
  return <p className="text-sm text-gray-800 leading-relaxed text-justify mb-3">{children}</p>
}

function PI({ children }: { children: React.ReactNode }) {
  return <p className="text-sm text-gray-800 leading-relaxed text-justify mb-2 pl-5">{children}</p>
}
